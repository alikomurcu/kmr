...
{
    template <>
    struct hash<kmr::KmrModel::Vertex>
    {
        size_t operator()(kmr::KmrModel::Vertex const &vertex) const
        {
            size_t seed = 0;
            kmr::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
            return seed;
        }
    };
} // namespace std
...
namespace kmr
{
    ...
    std::unique_ptr<KmrModel> KmrModel::createModelFromFile(
        KmrDevice &device,
    ...
    void KmrModel::Builder::loadModel(const std::string &filepath)
    {
